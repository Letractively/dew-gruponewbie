...
//SELECT id_avi, cargo_avi, inicio_avi, fin_avi, descripcion_avi, jornadaLaboral_avi, disponibilidad_avi, requerimientos_avi, beneficios_avi, sueldo_avi, estado_avi, conteoLectura_avi, id_per, id_Especialidad FROM tb_aviso'
	...
	public boolean modificarAviso(Aviso aviso) throws DAOExcepcion {
		boolean flagRpta=true;
		String query = "UPDATE tb_aviso SET cargo_avi = ?, inicio_avi = ?, fin_avi = ?, descripcion_avi = ?, jornadaLaboral_avi = ?, disponibilidad_avi = ?, requerimientos_avi = ?, beneficios_avi = ?, sueldo_avi = ?, estado_avi = ?, conteoLectura_avi = ?, id_Especialidad = ? WHERE id_avi = ?";
		Connection con = null;
		PreparedStatement stmt = null;
		try {
			con = ConexionBD.obtenerConexion();
			...
			stmt.setString(10, aviso.getEstado());
			stmt.setInt(11, aviso.getContadorVisitas());
			stmt.setInt(12, aviso.getEspecialidad().getIdEspecialidad());
			stmt.setInt(13, aviso.getId());
